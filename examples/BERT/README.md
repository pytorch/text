...
Moreover, [PowerSGD](https://arxiv.org/abs/1905.13727) gradient compression can be supported on top of DDP, by setting '--comm_hook_type' flag as either 'PowerSGD' or 'BatchedPowerSGD' (a faster variant without much accuracy loss when matrix approximation rank is 1). Another three flags ('--matrix_approximation_rank', '--start_powerSGD_iter', and '--min_compression_rate') can be used for tuning PowerSGD algorithm, as explained in [PyTorch Tutorial](https://pytorch.org/docs/master/ddp_comm_hooks.html#torch.distributed.algorithms.ddp_comm_hooks.powerSGD_hook.PowerSGDState). Additionally, '--use_fp16_compress_wrapper=1' can apply FP16 gradient compression on top of PowerSGD and further accelerate training if the input data format is FP32. Note that both '--min_compression_rate' and '--use_fp16_compress_wrapper' flags are ONLY available after PyTorch 1.8, so it is not available in the latest binary version by 2020 March. Therefore, currently the corresponding code is commented out in [cross_lingual_mlm_task.py](https://github.com/pytorch/text/blob/xlmr_mlm/examples/BERT/cross_lingual_mlm_task.py).

    srun --label --ntasks-per-node=1 --time=4000 --mem-per-cpu=5120 --gres=gpu:8 --cpus-per-task 80 --nodes=1 --pty python cross_lingual_mlm_task.py --num_lines 10000 --world_size 8 --pipeline_mode DDP --epochs 12 --lr 0.01 --bptt 96 --log-interval 50 --comm_hook_type PowerSGD --matrix_approximation_rank=1 --start_powerSGD_iter=5000
