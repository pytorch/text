...
class TestVocab(TorchtextTestCase):
    ...
    def test_vocab_basic_get(self):
        c = Counter({"hello": 4, "world": 3, "ᑌᑎIᑕOᗪᕮ_Tᕮ᙭T": 5, "freq_too_low": 2})
        v = vocab.Vocab(c, min_freq=3, specials=["<unk>", "<pad>", "<bos>"])

        self.assertEqual(v["world"], 5)
        self.assertEqual(v["hello"], 4)
        self.assertEqual(v["ᑌᑎIᑕOᗪᕮ_Tᕮ᙭T"], 3)
        self.assertEqual(v["<bos>"], 2)
        self.assertEqual(v["<pad>"], 1)
        self.assertEqual(v["freq_too_low"], 0)  # Unk
        self.assertEqual(v["asdf"], 0)  # Unk
    ...
    def test_vocab_specials_first(self):
        ...
        v = vocab.Vocab(
            c, max_size=2, specials=["<pad>", "<eos>"], specials_first=False
        )
    ...
    def test_vocab_set_vectors(self):
        c = Counter(
            {"hello": 4, "world": 3, "ᑌᑎIᑕOᗪᕮ_Tᕮ᙭T": 5, "ｔｅｓｔ": 4, "freq_too_low": 2}
        )
        v = vocab.Vocab(c, min_freq=3, specials=["<unk>", "<pad>", "<bos>"])
    ...
    def test_vocab_download_fasttext_vectors(self):
        ...
        if os.environ.get("TRAVIS") == "true":
            vec_file = os.path.join(
                self.project_root, ".vector_cache", "wiki.simple.vec"
            )
    ...
    def test_vocab_download_custom_vectors(self):
        ...
        for _ in range(2):
            v = vocab.Vocab(
                ...
                vectors=Vectors(
                    "wiki.simple.vec", url=FastText.url_base.format("simple")
                ),
            )

            self.assertEqual(
                v.itos, ["<unk>", "<pad>", "<bos>", "ᑌᑎIᑕOᗪᕮ_Tᕮ᙭T", "hello", "world"]
            )
            vectors = v.vectors.numpy()
    ...
    def test_vocab_download_glove_vectors(self):
        ...
        for i in range(3):
            ...
            for word in expected_twitter:
                assert_allclose(vectors[v.stoi[word], :5], expected_twitter[word])
        ...
        if os.environ.get("TRAVIS") == "true":
            zip_file = os.path.join(
                self.project_root, ".vector_cache", "glove.twitter.27B.zip"
            )
    ...
    def test_vocab_download_charngram_vectors(self):
        ...
        for i in range(3):
            ...
            for word in expected_charngram:
                assert_allclose(vectors[v.stoi[word], :5], expected_charngram[word])
    ...
    def test_vectors_get_vecs(self):
        ...
        token_one_vec = vec.get_vecs_by_tokens(
            tokens[0], lower_case_backup=True
        ).numpy()
        ...
        # Delete the vectors after we're done to save disk space on CI
        if os.environ.get("TRAVIS") == "true":
            zip_file = os.path.join(self.project_root, ".vector_cache", "glove.6B.zip")
            conditional_remove(zip_file)
