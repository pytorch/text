    ...
    with open(CACHE_STATUS_FILE, "r") as f:
        ...
        if missing_datasets:
            raise FileNotFoundError(
                "Failing all raw dataset unit tests as cache is missing {} datasets".format(missing_datasets)
            )
    ...
    for info in raw_data_info:
        ...
        try:
            if dataset_name == "WMT14":
                dataset = torchtext.experimental.datasets.raw.DATASETS[dataset_name](split=split)
            else:
                dataset = torchtext.datasets.DATASETS[dataset_name](split=split)
