    ...
    with open(CACHE_STATUS_FILE, "r") as f:
        ...
        if missing_datasets:
            raise FileNotFoundError(
                "Failing all raw dataset unit tests as cache is missing {} datasets".format(
                    missing_datasets
                )
            )
    ...
    for info in raw_data_info:
        ...
        try:
            if dataset_name == "WMT14":
                _ = torchtext.experimental.datasets.raw.DATASETS[dataset_name](
                    split=split
                )
            else:
                _ = torchtext.datasets.DATASETS[dataset_name](split=split)
            cache_status[dataset_name][split] = {
                "status": "success",
