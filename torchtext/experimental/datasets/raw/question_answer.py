...
def _setup_datasets(dataset_name, root, data_select, offset, stride):
    ...
    return tuple(RawTextIterableDataset(dataset_name, NUM_LINES[dataset_name][item],
                 _create_data_from_json(extracted_files[item]), offset=offset, stride=stride) for item in data_select)


def SQuAD1(root='.data', data_select=('train', 'dev'), offset=0, stride=1):
    ...
    return _setup_datasets("SQuAD1", root, data_select, offset, stride)


def SQuAD2(root='.data', data_select=('train', 'dev'), offset=0, stride=1):
    ...
    return _setup_datasets("SQuAD2", root, data_select, offset, stride)
