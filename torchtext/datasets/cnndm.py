...
if is_module_available("torchdata"):
    from torchdata.datapipes.iter import FileOpener, IterableWrapper, StreamReader, OnlineReader, FileLister, GDriveReader
...
STORIES_MD5 = {
    'cnn': '85ac23a1926a831e8f46a6b8eaf57263',
    'dailymail': 'f9c5f565e8abe86c38bfa4ae8f96fd72'
}
...
def _get_stories(root:str, source: str):
    ...
    cache_decompressed_dp = (
        FileOpener(cache_decompressed_dp, mode="b").load_from_tar()
    )
...
def CNNDM(root: str, split: Union[Tuple[str], str]):
    ...
    for story in story_fnames:
        ...
        if os.path.join(root, _EXTRACTED_FILES['cnn'], story) in cnn_stories:
            ...
            print(f"Error: Couldn't find story file {story} in either cnn or dailymail directories. Was there an error when loading the files?")
